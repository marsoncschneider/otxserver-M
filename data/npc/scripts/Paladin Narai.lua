...
done3 = 'Congratulations now you are paladin, Go to the dungeon and take your items and hunt.'
 local function greetCallback(cid)
	local player = Player(cid)
	local level = player:getLevel()
	
		npcHandler:setMessage(MESSAGE_GREET, player:getName() ..", Do you Want to become a Paladin? {Yes}?")
	...
	elseif msgcontains(msg, 'paladin') and talk_state == 1 then
		talk_state = 0
			...
			doPlayerSetVocation(cid, 3)
                       player:setStorageValue(43211, 3)
